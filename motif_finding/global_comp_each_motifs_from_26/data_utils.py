...
def load_recomb_data(aug):
    ...
    for chr_index in [str(1),str(2),str(3),str(4),str(5),str(6),str(7),str(8),str(9),str(10),str(11),str(12),str(13),str(14),str(15),str(16),str(17),str(18),str(19),str(20),str(21),str(22),'X']:
        current_chr_hot_sequences = load_fasta_gz("hotspots_AUROC_train_chr"+chr_index+".fasta.gz")
        hot_sequences += current_chr_hot_sequences
        current_chr_cold_sequences = load_fasta_gz("coldspots_AUROC_train_chr"+chr_index+".fasta.gz")
        cold_sequences += current_chr_cold_sequences

    if aug:
        X = np.array([one_hot(seq) for seq in hot_sequences] + [one_hot(reverse_complement(seq)) for seq in hot_sequences] + [one_hot(seq) for seq in cold_sequences] + [one_hot(reverse_complement(seq)) for seq in cold_sequences])[:,:301] # to make the pooling symmetric 
        Y = np.array([1]*(2*len(hot_sequences)) + [0]*(2*len(cold_sequences)))
    else:
        X = np.array([one_hot(seq) for seq in hot_sequences] + [one_hot(seq) for seq in cold_sequences])[:,:301] # to make the pooling symmetric 
